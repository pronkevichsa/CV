...
namespace Protocol
{
	...
	public class CProtocol  
	{
        ...
        #region structures
        //public struct Energy30
        //{
        //    public int[,] Impuls;// = new int[Canals, 48];
        //    public DateTime DataIn;
        //    public DateTime[] TimeIn;//=new DateTime[48];
        //    public double[,] energy30; //= new double[Canals, 48]; 
        //}
        //public struct Power30
        //{
        //    double[,] power30; //= new double[Groups, 48];
        //    DateTime DataIn;
        //    DateTime[] TimeIn;//=new DateTime[48];
        //}
        //public struct Power3
        //{
        //    double power3;
        //} 
       #endregion
    ...
    */
		...
		{ 
			...
			for (int i=0;i<5;i++)
			{
				//вырезаем подстроку и конвертируем в double
    ...
    //        test = serialPort1.ReadByte();
        ...
        protected int Com7(ref byte[] command,ref int[] Esc_input)
        {
           // string aaa = "";
            ...
            {
                test = serialPort1.ReadByte();
              //  input[i] = Convert.ToChar(test, 16);
                if (i == 6) break;
                if (test < 10)
                {
                 //   aaa = aaa + "0" + Convert.ToString(test, 16);
                ...
                {
                 //   aaa = aaa + Convert.ToString(test, 16);
      ...
      //      devide24(aaa, ref Esc_input);
       ...
       //     int test = serialPort1.ReadByte();
        ...
        public int SendCommand(char command, ref double[] export)
        {
            ...
            //    case '$': test = Com1152(ref com_dollar, ref export); break;
        ...
        public int TimeCorrection(ref CProtocol exportP)
        {
            ...
            int[] EscT = new int[7];            //= new char[7];
            int[] NowT = new int[7];
            int k;
            System.Threading.Thread.Sleep(1000 * (59 - st.second));


        //    CProtocol exportP = new CProtocol();
        //    exportP.OpenComPort();

            k = exportP.SendCommand('T', ref EscT);
            //      MessageBox.Show(Convert.ToString(EscT[1]));
            //time correction;
...
/// ///////////////////////////////////////////////////////
        ...
        public int SendDollar(ref double[,] Esc_input)
        {
            ...
            //{
            //    if (inp[i] < 16) aaa = aaa + "0" + Convert.ToString(inp[i], 16);
            //    else aaa = aaa + Convert.ToString(inp[i], 16);
            //}
            //int j = aaa.Length;
            //devide1152(aaa, ref Esc_input);
